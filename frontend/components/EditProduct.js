...
const EditProduct = ({ show, onClose, productId }) => {
    ...
    const [isIntValid, setIsIntValid] = useState(true);
    ...
    useEffect(() => {
        // Fetch product information when component mounts
        if (productId) {
            axios.get(`http://localhost:8086/products/${productId}`)
                .then(response => {
                    console.log(response.data);
                    const productData = response.data; // Assuming the response contains product information
                    setFormData({
...
//   };
  ...
  const handleEditProduct = (event) => {
    ...
    if (isStringValid && isIntValid) {
      axios
        .put(`http://localhost:8086/products/${productId}`, formData, {
          headers: {
        ...
        .then((response) => {
          if (response.status === 200) {
            const successMessage = response.data; // Assuming the response is a string like "Successfully!"
      ...
      <div className=" relative w-full max-w-2xl max-h-screen mx-auto my-6 overflow-y-auto">
        <div className="border rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
            ...
            <form onSubmit={handleEditProduct} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div className="mb-4">
